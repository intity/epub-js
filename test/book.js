...
describe("Book(unarchived)", () => {
	...
	describe("open book from package.json of external server", () => {
		const book = new Book("https://intity.github.io/epub-js/assets/alice/OPS/package.json")
		it("should open a epub", async () => {
			await book.opened
			assert.equal(book.isOpen, true)
			assert.equal(book.archived, false)
			assert.equal(book.url.toString(), "https://intity.github.io/epub-js/assets/alice/OPS/package.json")
		})
		it("should have a local coverUrl", async () => {
			const coverUrl = await book.coverUrl()
			assert.equal(coverUrl, "https://intity.github.io/epub-js/assets/alice/OPS/images/cover_th.jpg")
