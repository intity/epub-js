...
2. [Scrolling a document](scrolled.html)

    >Feature status: `stable`

    >Metadata value: `scrolled`
...
3. [Continuous scrolling of documents](scrolled-doc.html)
...
4. [Layout](layout.html)

    >Feature status: `develop`

    This example combines examples (1,2,3) in one. Basically, this example is needed to test layout features.

### viewport only

5. [Continuous swiping of pages](paginated-continuous.html)

    >Feature status: `stable`

    >Metadata value: `paginated`

    This example is for devices that support the touch events API 
    (see [Using Touch Events](https://developer.mozilla.org/en-US/docs/Web/API/Touch_events/Using_Touch_Events)).

6. [Continuous scrolling of documents](scrolled-continuous.html)
...
15. [Storage from directory](storage-unarchive.html)

    >Feature status: `stable`

    This example is similar to example (14). However, in this case, the book is opened from a directory.

## Other means of rendering

16. [Contents](contents.html)

    This example demonstrates the `innerText` rendering from range, which was obtained from EpubCFI format string.

17. [Renderless](renderless.html)
